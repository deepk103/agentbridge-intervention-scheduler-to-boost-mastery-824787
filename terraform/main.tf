...
resource "azurerm_resource_group" "main" {
  name     = "rg-agentbridge-${var.environment}-intervention-scheduler-to-boost-mastery"
  location = var.region
  tags = {
    Environment = var.environment
    ManagedBy   = "terraform"
    Blueprint   = "Intervention Scheduler to boost mastery"
  }
}

# Virtual Network
resource "azurerm_virtual_network" "main" {
  name                = "vnet-agentbridge-intervention-scheduler-to-boost-mastery"
...
resource "azurerm_subnet" "main" {
  name                 = "snet-agentbridge-intervention-scheduler-to-boost-mastery"
  resource_group_name  = azurerm_resource_group.main.name
...
resource "azurerm_network_security_group" "main" {
  name                = "nsg-agentbridge-intervention-scheduler-to-boost-mastery"
  location            = azurerm_resource_group.main.location
  resource_group_name = azurerm_resource_group.main.name
...
resource "azurerm_public_ip" "main" {
  ...
  name                = "pip-agentbridge-intervention-scheduler-to-boost-mastery"
  location            = azurerm_resource_group.main.location
  resource_group_name = azurerm_resource_group.main.name
...
resource "azurerm_network_interface" "main" {
  name                = "nic-agentbridge-intervention-scheduler-to-boost-mastery"
  location            = azurerm_resource_group.main.location
  resource_group_name = azurerm_resource_group.main.name
...
resource "azurerm_linux_virtual_machine" "main" {
  name                  = "vm-agentbridge-intervention-scheduler-to-boost-mastery"
  resource_group_name   = azurerm_resource_group.main.name
  location              = azurerm_resource_group.main.location
...
resource "azurerm_managed_disk" "data" {
  ...
  name                 = "disk-agentbridge-intervention-scheduler-to-boost-mastery-data"
  location             = azurerm_resource_group.main.location
  resource_group_name  = azurerm_resource_group.main.name
  storage_account_type = "Premium_LRS"
  create_option        = "Empty"
...
resource "azurerm_cognitive_account" "ai_services" {
  ...
  name                  = "agentbridge-intervention-scheduler-to-boost-mastery-ai"
  location              = azurerm_resource_group.main.location
  resource_group_name   = azurerm_resource_group.main.name
  kind                  = "AIServices"
  sku_name              = "S0"
  custom_subdomain_name = "agentbridge-intervention-scheduler-to-boost-mastery-ai"
...
resource "azurerm_user_assigned_identity" "vm_foundry" {
  count               = var.enable_foundry ? 1 : 0
  name                = "agentbridge-intervention-scheduler-to-boost-mastery-vm-identity"
  location            = azurerm_resource_group.main.location
  resource_group_name = azurerm_resource_group.main.name
}
...
resource "null_resource" "foundry_agents" {
  ...
  provisioner "local-exec" {
    interpreter = ["bash", "-c"]
    command     = <<-EOT
      ENDPOINT="https://agentbridge-intervention-scheduler-to-boost-mastery-ai.services.ai.azure.com"
